...
function exportData(type) {
    ...
    if (type === 'psych') exportClinicalLog();
...
function exportClinicalLog() {
    const { counts, confByEmo, total, totalConf } = aggregateSessionStats();
    const avgConf     = (totalConf / total * 100).toFixed(1);
    const dominant    = Object.entries(counts).sort((a,b) => b[1]-a[1])[0];
    const dominantPct = (dominant[1] / total * 100).toFixed(1);
    const now         = new Date();

    let content = `CLINICAL FACIAL EXPRESSION ANALYSIS REPORT\n${'='.repeat(50)}\n\n`;
    content += `Date: ${now.toLocaleDateString([],{year:'numeric',month:'long',day:'numeric'})}\n`;
    content += `Time: ${now.toLocaleTimeString([],{hour:'2-digit',minute:'2-digit'})}\n`;
    content += `Total Observations: ${total}\nAvg Confidence: ${avgConf}%\n\n`;
    content += `SESSION SUMMARY\n${'-'.repeat(50)}\nDominant Emotion: ${dominant[0]} (${dominantPct}%)\n\n`;
    ...
    content += `\n${'='.repeat(50)}\nGenerated by Ensemble CNN-CatBoost FER System\nFor professional use only\n`;
...
async function exportPDF() {
    try {
        ...
        // ── Gantt (div-based) via html2canvas ──
